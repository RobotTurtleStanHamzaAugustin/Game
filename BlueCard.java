class BlueCard extends Card {
    ...
    public void executerCard(Player player, Grid grid, Game game) {
        ...
        if (player.getDirection() == 0) {
            if (grid.presenceBlock(player.getPosition()[0] - 1, player.getPosition()[1])) {
                player.setDirection(2);
            } else if (grid.presenceJoueur(player.getPosition()[0] - 1, player.getPosition()[1])) {
                player.retourDepart();
                game.getPlayers().get(grid.indiceJoueur(player.getPosition()[0] - 1, player.getPosition()[1])).retourDepart();
            } else {
            ...
            if (grid.presenceBlock(player.getPosition()[0], player.getPosition()[1] + 1)) {
                player.setDirection(3);
            } else if (grid.presenceJoueur(player.getPosition()[0], player.getPosition()[1] + 1)) {
                player.retourDepart();
                game.getPlayers().get(grid.indiceJoueur(player.getPosition()[0], player.getPosition()[1] + 1)).retourDepart();
            } else {
